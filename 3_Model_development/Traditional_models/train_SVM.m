function [Mdl,Y,Y_mean,Y_best_mean,Stat,Stat_Y_mean,Stat_best_mean] = train_SVM(X,T,index, BoxConstraint, KernelScale, Epsilon, KernelFunction, num_try)
num_Test = length(index.Test);

% Initiate the outputs
Mdl.svm = cell(num_try,1);
...
for i = 1:num_try  
    % Train the SVM model
    svm = fitrsvm(X_Train_Validation,T_Train_Validation,...
        "BoxConstraint",BoxConstraint,"KernelScale",KernelScale,"Epsilon",Epsilon,"KernelFunction",KernelFunction);

    % Get Y
    Y_Train_Validation = predict(svm,X_Train_Validation);
    Y_Test = predict(svm,X_Test);

    % Assign outputs
    % Mdl.svm{i} = svm; % Here do not save the model becuase it is too large
    Y.Test(:,i) = Y_Test;
    ...
    disp(['BoxConstraint: ',num2str(BoxConstraint),', KernelScale: ',num2str(KernelScale),', Epsilon: ',num2str(Epsilon)])
