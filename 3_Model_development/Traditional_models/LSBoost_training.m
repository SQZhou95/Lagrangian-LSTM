...
%% Set hyperparameters for LSBoost
para.NumLearningCycles = 500;
para.MinLeafSize = 1000;
para.LearnRate = 0.1;

save('LSBoost_training.mat','X','T','time_valid','index','para')

%% Training

[~,Y,Y_mean,Y_best_mean,Stat,Stat_Y_mean,Stat_best_mean] = train_LSBoost(X,T,index, para.NumLearningCycles, para.MinLeafSize, para.LearnRate, num_try);
LSBoost.results = struct( ...
    'Y', Y, ...
    'Y_mean', Y_mean, ...
...
disp(LSBoost.bestResults)


save('LSBoost_training.mat','LSBoost','-append')
