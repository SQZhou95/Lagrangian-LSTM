% Grid search for hyperparameters of LSBoost
...
%% Set hyperparameters for Grid_CV of LSBoost
...
save('LSBoost_grid_search.mat','X','T','time_valid','index','grid_search','-v7.3')

%% Initiate the training results

LSBoost.results = [];
LSBoost.stat_best_mean = [];
...
for i = 1:length(grid_search.NumLearningCycles)
    ...
    for j = 1:length(grid_search.MinLeafSize)
        ...
        for k = 1:length(grid_search.LearnRate)
            LearnRate = grid_search.LearnRate(k);

            [Mdl,Y,Y_mean,Y_best_mean,Stat,Stat_Y_mean,Stat_best_mean] = train_LSBoost(X,T,index, NumLearningCycles, MinLeafSize, LearnRate, num_try);
            LSBoost.results = [LSBoost.results; struct( ...
            ...
            LSBoost.stat_best_mean = [LSBoost.stat_best_mean; struct( ...
                'NumLearningCycles', NumLearningCycles, ...
                'MinLeafSize', MinLeafSize, ...
                'LearnRate', LearnRate, ...
            ...
            disp(struct2table(LSBoost.stat_best_mean))
...
save('LSBoost_grid_search.mat','LSBoost','bestModel','-append')
