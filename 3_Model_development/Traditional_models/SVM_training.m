...
para.BoxConstraint = 100;
para.KernelScale = 100;
para.Epsilon = iqr(T)/1.349;
para.KernelFunction = 'polynomial';

save('SVM_training.mat','X','T','time_valid','index','para')

%% Training

[~,Y,Y_mean,Y_best_mean,Stat,Stat_Y_mean,Stat_best_mean] = train_SVM(X,T,index, para.BoxConstraint, para.KernelScale, para.Epsilon, para.KernelFunction, num_try);
SVM.results = struct( ...
    'Y', Y, ...
    'Y_mean', Y_mean, ...
...
disp(SVM.bestResults)


save('SVM_training.mat','SVM','-append')
