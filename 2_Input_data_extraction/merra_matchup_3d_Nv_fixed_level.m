function data_traj = merra_matchup_3d_Nv_fixed_level_new(dataPath,TrajData,TrajLength,Traj_time,FieldName,Start_time,fixed_pressure)
% Start_time: the start time (hour) in each file. It's 0 for instantaneous,
% but 1.5 for tine-averaged files
...
%                 data = cat(4,data,ncread(fullfile(dataPath,Files(f).name),FieldName,[1 1 id_lev 1],[Inf Inf 1 Inf]));
...
%             data = cat(4,data(:,:,:,9:end),ncread(fullfile(dataPath,Files(i+(TrajLength+2)).name),FieldName,[1 1 id_lev 1],[Inf Inf 1 Inf]));
    ...
    for j = 1:length(id)
        ...
        data_traj{id(j)}(:,1) = flipud(data_matchup(data,lev,TrajLength,Date_start,Traj_temp,Start_time,fixed_pressure));
    end
...
%
function data_point = data_matchup(data,lev,TrajLength,Date_start,Traj,Start_time,fixed_pressure)
